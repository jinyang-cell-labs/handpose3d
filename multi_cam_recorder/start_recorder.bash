#!/usr/bin/env bash
#
# Bootstrap + launch the multi-camera recorder GUI.
#   - finds (or creates) a local .venv next to this script
#   - installs requirements.txt on first run (or with --reinstall)
#   - launches the PyQt recorder with config.yaml (or a custom config)
#
# Usage:
#   ./start_recorder.bash                       # normal launch
#   ./start_recorder.bash my_config.yaml        # custom config
...
echo ">> launching multi-camera recorder"
exec python "$SCRIPT_DIR/recorder.py" "${ARGS[@]+"${ARGS[@]}"}"
