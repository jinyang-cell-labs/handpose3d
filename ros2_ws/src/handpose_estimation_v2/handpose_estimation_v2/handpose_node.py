...
"""
Hand pose estimation node, v2: model-based fit (no per-joint triangulation).

Front-end is identical to handpose_estimation's handpose_node: two calibrated
camera streams, MediaPipe HandLandmarker per view for 2D keypoints, hands
matched across cameras by handedness label.

The estimation core differs (docs/estimation_guide_v1.md stages A + C are
replaced): instead of triangulating each joint independently (A) and fitting
the rigid template to the 3D points afterwards (C), v2 estimates the
template's 6-DoF pose DIRECTLY by minimising robust 2D reprojection error of
all 21 template joints in both views simultaneously:

    2D keypoints (both views) ->
    rigid template fit on reprojection error (LM, Huber, B2 RANSAC in px) ->
    B1 reachability gate (fitted wrist) ->
    D constant-velocity Kalman + SLERP LPF -> E One-Euro polish

Consequences of the model-based formulation:
  * the template's bone lengths/scale constrain depth inside the solve
    (no unconstrained two-ray intersections per joint);
  * the fit is warm-started from the previous frame's pose (temporal
    coupling at the measurement level);
  * a hand seen in only ONE camera can still be tracked (monocular PnP
    continuation) instead of coasting.

Outputs match v1: wrist PoseStamped per hand, skeleton MarkerArray (the raw
DLT triangulation can still be published for A/B against the fitted
skeleton), optional annotated 2D views.
"""
...
from handpose_estimation.wrist_pose_pipeline import ReachabilityShell
...
# Marker ids for the fitted-template skeleton (R*template+t, the v2 output),
# published alongside the raw triangulation for A/B comparison in RViz.
...
class HandPoseNodeV2(Node):
    def __init__(self):
        super().__init__("handpose_node_v2")

        # --- parameters -----------------------------------------------------
        self.declare_parameter("camera_names", ["camera0", "camera1"])
        # Model + template are shared with v1 (source-tree paths, like v1).
        ...
        # true  -> fit against the camera_info stereo calibration
        #          (undistort/rectify with K/D/R/P; rectified P is metric).
        # false -> raw K + extrinsics.yaml (no distortion handling).
        ...
        if len(self.camera_names) != 2:
            raise ValueError("handpose_node_v2 requires exactly 2 camera_names")
        ...
        # --- model-fit pipeline parameters -----------------------------------
        self._declare_pipeline_params()
        self._load_template_and_shell()
        # Trackers are built once calibration arrives: the fit needs
        # effective_scale (world units) which depends on the mode.
        self.trackers = None
        ...
        # it when fitting against the camera_info stereo calibration.
        self.extrinsics = (
            None
        ...
        # the projection mode is chosen once both have arrived.
        ...
        # 6-DoF wrist pose per hand (model-fit pipeline output).
        self.wrist_pubs = {
        ...
        self.get_logger().info(
            f"handpose_node_v2 (model-based fit) ready: cameras={self.camera_names}, "
    ...
    # --------------------------------------------- model-fit pipeline setup
    def _declare_pipeline_params(self):
        """Declare and read the model-fit pipeline parameters.

        All lengths are metres unless suffixed _px (pixel thresholds of the
        reprojection-domain fit).
        """
        defaults = {
            "nominal_fps": 30.0,
            ...
            # Keep publishing the raw per-joint DLT triangulation (v1 Stage A)
            # purely for RViz A/B comparison against the fitted skeleton.
        ...
        self.pipe_flags = {
            "model_fit": bool(self.pipe_params["model_fit.enabled"]),
            "single_view": bool(self.pipe_params["single_view.enabled"]),
        ...
        self.publish_raw_triangulation = bool(
            self.pipe_params["publish_raw_triangulation"]
        )
    ...
    def _build_trackers(self):
        """One ModelFitWristTracker per hand (left gets a y-mirrored template).

        Called from _on_calibration_ready: the fit runs in the projection
        matrices' world units, so it needs effective_scale.
        """
        self.trackers = {}
        for i, hand in enumerate(HAND_LABELS):
            ...
            self.trackers[hand] = ModelFitWristTracker(
                handedness=hand,
                template=tmpl,
                ...
                rng_seed=i,
                world_scale=self.effective_scale,
            )
    ...
    def _on_calibration_ready(self):
        """Both calibrations are in — set the projection mode + build trackers."""
        if self.use_camera_info_extrinsics:
            self.mode = "stereo"
            ...
            if baseline <= 1e-9:
                self.get_logger().warn(
                    ...
                    "stereo-calibrated. The multi-view fit will be degenerate."
                )
            else:
                self.get_logger().info(
                    "Projection mode: STEREO from camera_info "
            ...
            self.get_logger().info(
                "Projection mode: EXTRINSICS (extrinsics.yaml + raw K)."
            )
    ...
    def _on_images(self, *msgs):
        # Calibration on both cameras must arrive before we can fit.
        if not self.ready:
            self.get_logger().warn(
        ...
        # Projection matrices the 2D observations are fit against.
        if self.mode == "stereo":
            projections = [self.calib[n]["p"] for n in self.camera_names]
        else:
            projections = [self.P_ext[n] for n in self.camera_names]

        # Match hands across cameras by handedness label; feed every view's
        # 2D keypoints to the model fit (one view is enough with a live track).
        ...
        for label in HAND_LABELS:
            observations = []
            view_weights = np.zeros(len(self.camera_names))
            for i, name in enumerate(self.camera_names):
                kp = hands_2d[i].get(label)
                if kp is None:
                    observations.append(np.full((N_LANDMARKS, 2), np.nan))
                    continue
                pts = kp
                if self.mode == "stereo":
                    und = np.full((N_LANDMARKS, 2), np.nan)
                    valid = np.all(np.isfinite(kp), axis=1)
                    if valid.any():
                        und[valid] = self._undistort_points(name, kp[valid])
                    pts = und
                observations.append(pts)
                view_weights[i] = scores_2d[i][label]

            present = view_weights > 0.0
            agg_conf = (
                float(np.clip(np.mean(view_weights[present]), 0.5, 1.0))
                if present.any() else 0.0
            )
            res = self.trackers[label].update(
                stamp_sec, observations, projections, view_weights, agg_conf
            )
            self._publish_wrist_pose(label, res, stamp)

            if self.pipe_params["model_fit.publish_debug"] and res is not None \
                    and res["valid"]:
                self.get_logger().info(
                    f"{label} fit: rmse={res['rmsd']:.2f}px "
                    f"inliers={res['n_inliers']} views={int(present.sum())}",
                    throttle_duration_sec=1.0,
                )

            # Raw per-joint DLT triangulation, ONLY for RViz A/B comparison
            # (the estimation no longer depends on it).
            if self.publish_raw_triangulation and present.sum() >= 2:
                points_3d_by_hand[label] = self._triangulate_raw(
                    observations, projections, view_weights
                )

            # Fitted-template skeleton (R*template+t, metres): the v2 output.
            if (
        ...
        self.get_logger().info(
            ...
            f"-> fitted {sorted(fitted_by_hand)}",
            throttle_duration_sec=5.0,
        )
    ...
    def _triangulate_raw(self, observations, projections, view_weights):
        """Per-joint DLT (v1 Stage A) for the RViz comparison markers only.

        Returns (21, 3) world-unit points (NaN where missing in either view).
        """
        points_3d = np.full((N_LANDMARKS, 3), np.nan)
        finite = np.stack(
            [np.all(np.isfinite(o), axis=1) for o in observations]
        )
        both = finite.sum(axis=0) >= 2
        for j in np.where(both)[0]:
            views = np.where(finite[:, j])[0]
            X, _, _ = triangulate_point(
                [projections[i] for i in views],
                [observations[i][j] for i in views],
                view_weights[views],
            )
            points_3d[j] = X
        return points_3d
    ...
    def _publish_wrist_pose(self, label, res, stamp):
        ...
        ``res`` is the tracker output; None (track lost/reset) publishes
    ...
    def _publish_markers(self, points_3d_by_hand, stamp, fitted_by_hand=None):
        ...
        previous skeleton in RViz instead of leaving it stale. The fitted
        template skeleton (R*template+t, metres — the v2 estimate) is
        published alongside the raw triangulation for A/B comparison.
        """
        ...
        for label in HAND_LABELS:
            ...
            # Fitted-template skeleton (the model-fit output).
